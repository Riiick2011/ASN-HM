...
from utils.misc import CollateFunc, build_dataset, build_dataloader, seed_everything
...
def train():
    ...
    args.weight_folder = os.path.join(os.path.abspath('.'), args.weight_folder)  # 权重文件夹的绝对路径
    ...
    for epoch in range(start_epoch, args.max_epoch):  # 从0开始
        if args.distributed:
            dataloader.batch_sampler.sampler.set_epoch(epoch)  # 每个周期的随机种子是0+epoch

        # train one epoch
        for iter_i, (batch_img_name, batch_video_clips, batch_key_frame_target, batch_clip_target) in enumerate(dataloader):
            ...
            # inference
            batch_output = model(batch_video_clips, batch_clip_target)
            
            # loss
            loss_dict = criterion(batch_output, batch_key_frame_target)
